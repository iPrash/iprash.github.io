/* XYZ AI Engagement site — shared behaviour
   Storage keys are namespaced so the two documents track progress separately. */
(function () {
  'use strict';

  /* ---------- theme ---------- */
  var KEY_THEME = 'xyzai.theme';
  ...
  if (doc && docId) {
    ...
    function paint() {
      ...
      try { localStorage.setItem('xyzai.pct.' + docId, String(pct)); } catch (e) {}
    }
    paint();
  ...
  /* ---------- index page: renewal calculator ---------- */
  var rIn = document.getElementById('renewalDate');
  if (rIn) {
    var KEY_R = 'xyzai.renewal';
    var stored = null;
    try { stored = localStorage.getItem(KEY_R); } catch (e) {}
    if (stored) rIn.value = stored;

    var STEPS = [
      { d: 150, label: 'Data access granted', note: 'R − 150' },
      { d: 135, label: 'Baseline locked', note: 'R − 135' },
      { d: 120, label: 'Draft brief + BATNA costed', note: 'R − 120' },
      { d: 105, label: 'Brief approved', note: 'R − 105' },
      { d: 90, label: 'First negotiation session', note: 'R − 90' }
    ];
    ...
    function calc() {
      var box = document.getElementById('backdates');
      var status = document.getElementById('renewalStatus');
      if (!box) return;
      if (!rIn.value) {
        box.innerHTML = '<div class="bd"><span class="metric-label">Backward schedule</span>' +
          '<div class="val">Set the renewal date</div>' +
          '<div class="sub">Playbook §2.3 — every milestone runs backwards from it</div></div>';
        if (status) status.textContent = 'not set';
        return;
      }
      try { localStorage.setItem(KEY_R, rIn.value); } catch (e) {}
      var R = new Date(rIn.value + 'T12:00:00');
      var now = new Date(); now.setHours(12, 0, 0, 0);
      var daysOut = Math.round((R - now) / 86400000);
      if (status) status.textContent = daysOut + ' days out';
      var html = '';
      STEPS.forEach(function (s) {
        var when = new Date(R.getTime() - s.d * 86400000);
        var slip = Math.round((when - now) / 86400000);
        var late = slip < 0;
        html += '<div class="bd' + (late ? ' late' : '') + '">' +
          '<span class="metric-label">' + s.note + ' · ' + s.label + '</span>' +
          '<div class="val">' + fmt(when) + '</div>' +
          '<div class="sub">' + (late ? 'passed ' + Math.abs(slip) + 'd ago — compress' : 'in ' + slip + ' days') + '</div>' +
          '</div>';
      });
      if (daysOut < 150) {
        html += '<div class="bd late"><span class="metric-label">Plan check</span>' +
          '<div class="val">Compressed plan</div>' +
          '<div class="sub">Under 150 days — say so in writing on day 1</div></div>';
      }
      box.innerHTML = html;
    }
    rIn.addEventListener('change', calc);
    rIn.addEventListener('input', calc);
    calc();

    /* days until the Codex model retirement */
    var cd = document.getElementById('codexDays');
    if (cd) {
      var target = new Date('2026-08-31T12:00:00');
      var n2 = new Date(); n2.setHours(12, 0, 0, 0);
      var left = Math.round((target - n2) / 86400000);
      cd.textContent = left > 0 ? left + ' days left' : (left === 0 ? 'today' : 'passed');
      if (left <= 0) cd.classList.remove('hot');
    }

    /* progress readouts on the document cards */
    ['playbook', 'resources'].forEach(function (id) {
      var el = document.getElementById('pct-' + id);
      if (!el) return;
      var v = null;
      try { v = localStorage.getItem('xyzai.pct.' + id); } catch (e) {}
